...
## 10. 云桌面画质进一步优化空间评估（2026-07-24 05:26）

云桌面内容特征是"小字 + 锐利边缘 + 大面积静止"，与自然视频相反。基于当前配置（CBR 60fps、编码 720×1560、QP 上限、3s 周期 IDR、boost REMB×1.35/4.5M）评估剩余优化空间，按收益排序：

| # | 优化项 | 现状 | 方案 | 预期收益 | 成本/风险 |
|---|--------|------|------|---------|----------|
| 1 | **编码分辨率提到原生 1080p** | 物理 1080×2336 缩到 720×1560 编码，浏览器再放大——文字两次重采样，是桌面小字发虚的第一原因 | Resolution 选择器加 `1080p`（1080×2336）档；H.264 需 L5.0 或限 30fps（1080p60 超 L4.2 宏块率），HEVC 需 L4.1 | **最大**；也是绕开 4:2:0 色度欠采样对彩色文字边缘伤害的唯一手段 | LAN/P2P 带宽足够（HEVC 5~6M）；广域网需按 REMB 门禁 |
| 2 | **桌面场景降到 30fps** | 60fps 固定，REMB 预算摊薄，每帧只有一半比特 | 先单变量试 `override_fps=30`；进阶：用已有前台监测做"桌面 30fps / 视频 App 60fps"动态切换 | 每帧码率翻倍，滚动文字明显更实 | 配置一行即可试；滚动流畅度略降 |
| 3 | **放宽周期 IDR（3s→10s+）** | 每 3s 一个 30~110KB IDR，平均吃 100~300kbps（稳态 5~8%），且 IDR 比累积 P 帧糊，画面每 3s"呼吸"一次 | `iframe_interval=10`；恢复已由 NACK/PLI + 加入/场景切换强制 IDR 覆盖 | 省下比特全进 P 帧质量，消除周期性质量抖动 | 弱网长 GOP 恢复依赖 PLI（已有） |
| 4 | 静止后 BWE 爬坡 | 桌面静止发送码率极低，浏览器 BWE 衰减；恢复后 REMB 爬回稳态需 ~11s（23:53 日志） | 如回归后仍可感知：RTP padding 保温 BWE | 小（boost 已覆盖切换瞬间，影响的是其后 2~10s 的滚动） | libdatachannel 侧改动大，暂不建议 |

**不建议动**：稳态码率/`remb_max`（6M 从未打满）；`COLOR_RANGE_FULL`（渲染一致性风险大于收益）；继续收紧 QP 上限（当前值已接近合理区间，再紧会顶着 CBR 产生帧尺寸波动）。

**推荐落地顺序**：先回归当前待部署构建（§8.4）→ 试 `override_fps=30` → 加 1080p 档 → `iframe_interval` 3→10。每步单变量，主观对比 + 盯 REMB/NACK。
